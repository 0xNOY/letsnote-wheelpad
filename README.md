...
- **The input-proxy recovery changes are not yet hardware-validated.** Startup waiting and grab retry with contacts or buttons active, signal shutdown, `SYN_DROPPED` recovery, and button-held scrolling still require an end-to-end test with a physical touchpad, uinput, and libinput.
...
At startup, the virtual devices are created while the physical device remains ungrabbed. If any MT tracking ID, `BTN_TOUCH`, or physical touchpad button is active, the daemon waits so existing consumers can finish that lifecycle. It grabs only after observing a quiescent snapshot, immediately drains pre-grab events and re-queries the state, and releases/retries if input became active in the race window. Gesture and routing state is initialized from the confirmed quiescent snapshot, and systemd `READY=1` is sent only afterward. The daemon does not attempt to proxy a contact or button that was already active before the permanent grab.
